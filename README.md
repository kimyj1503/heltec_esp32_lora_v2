# Heltec ESP32 LoRa V2

## Attribution

> This repository is a modified fork of
> [ropg/heltec_esp32_lora_v3](https://github.com/ropg/heltec_esp32_lora_v3)
> by Rop Gonggrijp.
>
> The original project is licensed under the MIT License. Original copyright and
> license notices are preserved in the `LICENSE` file.

## Project Description
This fork has been modified and tested for the Heltec ESP32 LoRa V2.1,
also known as the Heltec WiFi LoRa 32 V2.

* [Official product page](https://heltec.org/project/wifi-lora-32v2/)
* [Schematic](https://github.com/user-attachments/files/29872137/WIFI_LoRa_32_V2.1.868-915.pdf)

<img width="300" height="300" alt="Heltec WiFi LoRa 32 V2 board" src="https://github.com/user-attachments/assets/ac9c3585-82a3-4cf9-b42a-7d39dc3b5e77" />
